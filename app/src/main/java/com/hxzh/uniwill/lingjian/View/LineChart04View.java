 ...
 * @author XiongChuanLiang<br/>(xcl_168@aliyun.com)
 */
public class LineChart04View extends DemoView {

	private String TAG = "LineChart04View";
...
//				ToastUtil.showToast("没有数据");
	...
	//X轴时间
	public void chartLabels(List<Data_Tubiaotongji.ListBean> list2,int tag) {
		if (tag == 1){
			if (list2!=null&&list2.size()>0){
				...
				for (int i = 0; i < list2.size(); i++) {
					labels.add((String) list2.get(i).getUsername());
				}
				this.invalidate();
			}
		}else if (tag == 2){
			if (list2!=null&&list2.size()>0){
				...
				for (int i = 0; i < list2.size(); i++) {
					labels.add((String) list2.get(i).getCount());
				}
				this.invalidate();
			}
		}
